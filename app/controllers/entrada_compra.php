...
class Entrada_compra extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('model_entrada_compra', 'model_entrada_compra');  
...
//***********************Todos los recepciones**********************************//
	...
	public function listado_entradas(){
		 if($this->session->userdata('session') === TRUE ){
		      ...
		      if ( (count($coleccion_id_operaciones)==0) || (!($coleccion_id_operaciones)) ) {
		       	...
		       	$data['consecutivo']  	= $this->catalogo->listado_consecutivo(71);
  ...
  function validar_agregar_producto_compra(){
    if ($this->session->userdata('session') !== TRUE) {
      redirect('');
    } else {
		...
		if (($d_conf['configuracion']->activo==1)) {  
			...
			$existe_factura = $this->model_entrada_compra->existencia_factura($data);
	...
	public function procesando_productos_temporales_compra(){
		$data=$_POST;
		$busqueda = $this->model_entrada_compra->buscador_productos_temporales($data);
		echo $busqueda;
	}	

/////////////////////
	//4- Eliminar los productos desde la regilla
	public function eliminar_prod_temporal_compra($id = '', $nombrecompleto=''){
	    if ( $this->session->userdata('session') !== TRUE ) {
	      redirect('');
	    } else {
	      $id_perfil=$this->session->userdata('id_perfil');
	...
	function validar_eliminar_prod_temporal_compra(){
		if (!empty($_POST['id'])){ 
			$data['id'] = $_POST['id'];
		}

		$dato = $this->model_entrada_compra->valores_reordenar($data );
		
		$eliminado = $this->model_entrada_compra->eliminar_prod_temporal(  $data );
		$reordenar = $this->model_entrada_compra->reordenar_prod_temporal( $dato );
...
/////////////////////
	...
	public function validar_proceso_compra(){ 
	...
	public function procesar_entrar_compra($num_mov,$id_factura,$id_estatus){ 
	...
	public function procesar_entrada_compra($id_movimiento=-1,$dev=0,$retorno,$id_factura,$id_estatus){
		 ...
		 if($this->session->userdata('session') === TRUE ){
		      ...
		      if (($existe) or ($id_movimiento!=-1) ) {
		      		...
		      		if (($id_movimiento)==-1)	{
		      			...
		      			$data['num_mov'] = $this->model_entrada_compra->procesando_operacion(1);
