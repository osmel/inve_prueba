<?php if(! defined('BASEPATH')) exit('No tienes permiso para acceder a este archivo');
  ...
  class model_traspaso extends CI_Model {
    ...
    //para tomar los valores del encabezado del "modulo_traspaso"
 ...
 public function buscador_entrada($data){
          ...
          $this->db->select('m.id, m.movimiento,m.movimiento_unico, m.id_fac_orig, m.id_descripcion,m.devolucion, m.num_partida');
          ...
          $this->db->select("prod.codigo_contable, m.nombre_usuario,m.id_compra, m.id_operacion, m.c234");  
...
//regilla 2 "modulo traspaso"
 public function buscador_salida($data){
          ...
          $this->db->select(' m.id, m.movimiento,m.movimiento_unico, m.id_fac_orig, m.id_descripcion, m.devolucion, m.num_partida');
          $this->db->select('m.cantidad_um,  m.ancho, m.precio, m.codigo,m.id_operacion, m.c234');
...
//////////eliminar y quitar producto
    //pasar de la regilla 1 a la regilla 2 producto
      ...
      //pasar de la regilla 2 a la regilla 1 producto    
    public function quitar_productos_traspasado( $data ){
          ...
          //cdo quita producto necesita saber total
   ...
   /////////// procesar
       ...
       //obtener el consecutivo del "traspaso_modulo" a procesar
       ...
       //obtener el consecutivo new del "traspaso_modulo" a procesar
 ...
 //procesando el traspaso en "modulo_traspaso"      
 public function procesando_traspaso_definitivo( $data ){
             $id_session = $this->session->userdata('id');
             $fecha_hoy = date('Y-m-d H:i:s');

             $this->db->select('"'.$id_session.'" AS id_usuario',false); 
             $this->db->select('"'.addslashes($data['consecutivo']).'" AS consecutivo_traspaso',false); //old
             $this->db->select('"'.addslashes($data['consecutivo_unico']).'" AS consecutivo_traspaso_unico',false); //new
             ...
             $this->db->select('id_operacion,id id_entrada, id_almacen, factura,  id_cargador, fecha_salida,   movimiento, movimiento_unico, id_empresa, id_descripcion, id_color, devolucion, num_partida');
       ...
       //parte del proceso del "modulo_traspaso"
    ...
    /////////////Impresion despues de confirmar el traspaso
    public function imprimir_traspaso_historico_detalle($data){
         ...
         ',False);  
          ...
          $this->db->select("m.consecutivo_traspaso,m.consecutivo_traspaso_unico");  
          ...
          $where = '(
                      ( ( m.id_factura =  '.$data["id_factura"].' ) AND
                        ( m.consecutivo_traspaso_unico =  '.$consecutivo_traspaso.' )
                      )
            )';   

          $this->db->where($where);
          $where_total = '( m.consecutivo_traspaso_unico =  '.$consecutivo_traspaso.')';
          //$this->db->order_by($columna, $order); 
...
public function totales_importes_traspaso_especifico($where){
 ...
 public function totales_campos_traspaso_especifico($where){
      ...
      public function buscador_traspaso_historico($data){
          ...
          $this->db->select("SQL_CALC_FOUND_ROWS(m.consecutivo_traspaso)"); 
          $this->db->select('m.id_apartado apartado');   
...
//$filtro ='';
          ...
          $this->db->group_by("m.consecutivo_traspaso_unico,m.id_factura,m.id_usuario_apartado, m.id_cliente_apartado,m.consecutivo_venta");
              ...
              if ( $result->num_rows() > 0 ) {
                  ...
                  foreach ($result->result() as $row) {
                            ...
                            $dato[]= array(
                                      ...
                                      6=>$num,  //consecutivo de apartado
                                      7=>$row->consecutivo_traspaso_unico,  //consecutivo de apartado
...
////Resumen de impresion 
  ...
  public function imprimir_traspaso_historico($data){
          ...
          $this->db->select('m.consecutivo_traspaso, m.consecutivo_traspaso_unico', FALSE);
          $this->db->select('m.mov_salida', FALSE);
...
public function totales_imprimir_traspaso_historico($data){
    ...
    public function buscador_traspaso_historico_detalle($data){
         ...
         ',False);  
          ...
          $where = '(
                      (
                        ( m.consecutivo_traspaso_unico =  '.$consecutivo_traspaso.' ) AND ( m.id_factura =  '.$data["id_factura"].' )
                      )
                   AND
                      (
                        ( CONCAT(m.cantidad_um," ",um.medida) LIKE  "%'.$cadena.'%" ) OR (CONCAT(m.ancho," cm") LIKE  "%'.$cadena.'%")  OR
              ...
              if ( $result->num_rows() > 0 ) {
                  ...
                  foreach ($result->result() as $row) {
                            ...
                            ///////////////////////////////
                              ...
                              $consecutivo_traspaso=$row->consecutivo_traspaso_unico;
...
//imprimir el traspaso interior que hay en  "detalle traspaso en proceso"
